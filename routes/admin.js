...
router.get("/eleicao/edit/:id", userLogin, (req, res) => {
  Eleicao.findOne({ _id: req.params.id })
    .populate("chapa")
    .then((eleicao) => {
...
router.post("/eleicao/edit/del/", userLogin, (req, res) => {
  Eleicao.remove({ _id: req.body.id })
    .then(() => {
...
router.post("/eleicao/edit/", userLogin, (req, res) => {
  Eleicao.findOne({ _id: req.body.id }).then((eleicao) => {
    Chapa.find().then((chapa) => {
      ...
      if (
        !req.body.local ||
        typeof req.body.local == undefined ||
        req.body.local == null
      ) {
      ...
      if (
        !req.body.chapa ||
        typeof req.body.chapa == undefined ||
        req.body.chapa == null
      ) {
...
router.post("/candidato/del", userLogin, (req, res) => {
  Candidato.remove({ _id: req.body.id })
    .then(() => {
      // res.redirect("/admin/eleicao")
      res.json({ ok: "deletok" });
    })
...
router.get("/candidato/edit/:id", userLogin, (req, res) => {
  Candidato.findOne({ _id: req.params.id })
    .then((candidato) => {
...
router.post(
  ...
  async (req, res) => {
    ...
    if (
      !req.body.candidatos ||
      typeof req.body.candidatos == undefined ||
      req.body.candidatos == null
    ) {
      erros.push({ text: "Selecione pelo menos um candidato" });
    }
...
router.get("/chapas/edit/:id", userLogin, (req, res) => {
  Chapa.findOne({ _id: req.params.id })
    .populate("candidatos")
    .then((chapa) => {
      Candidato.find().then((candidatos) => {
...
router.get("/eleitor", userLogin, (req, res) => {
  ...
  router.post("/eleitor/pass", userLogin, (req, res) => {
    Eleitor.findOne({ _id: req.body.id }).then((eleitor) => {
      ...
      Eleitor.updateOne({ _id: req.body.id }, { $set: { senha: NewPass } })
        .then(() => {
  ...
  fs.readFile("./uploads/file.csv", async (err, data) => {
    ...
    for (item in eleitor) {
      const novoEleitor = {
        ...
        situacao: eleitor[item]["situacao"],
        nascimento: eleitor[item]["nascimento"],
      };

      await new Eleitor(novoEleitor)
        .save()
        .then(() => {
